...
class City {
    ...
    func setup() {
        ...
        showMessage("Creating sky")
        sky = Sky(device: device)
        showMessage("Creating floor")
        floor = PlaneModel(device: device)

        cars = Cars(device:device)

        showMessage("Building city")
        buildCity()

        // Add in 100 cars
        showMessage("Creating cars")
        for _ in 0 ..< 100 {
            cars.addCar()
        }
...
//                            bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue)
    ...
    func buildRoads() {
        ...
        showMessage("   generating east/west roads")
        var y = WORLD_EDGE
        ...
        showMessage("   generating north/south roads")
        broadway_done = false
        var x = WORLD_EDGE
        ...
        showMessage("   generating east/west streetlights")
        ...
        showMessage("   generating north/south streetlights")
        ...
        showMessage("   Placing large buildings in center of map")
        ...
        showMessage("   Placing smaller buildings around outside")
        x = 0
        while x < WORLD_SIZE {
            var y = 0
            while y < WORLD_SIZE {
