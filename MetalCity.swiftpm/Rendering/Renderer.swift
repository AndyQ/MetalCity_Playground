...
class Renderer: NSObject, MTKViewDelegate {
    ...
    func updateSharedUniforms()
    {
        ...
        var uniforms = Uniforms(viewProjectionMatrix: modelViewProjectionMatrix)
//        uniforms.viewProjectionMatrix = modelViewProjectionMatrix
    ...
    func updateUniforms()
    {
        guard self.city.isInitialised else { return }
    ...
    func draw(in view: MTKView) {
        guard city.isInitialised else { return }
...
extension Renderer {
    ...
    func setAutoCam( mode: CameraBehaviour ) {
        self.autoCam.setCameraBehaviour(behaviour:  mode)
        self.autoCam.timeUntilNextChange = 0
    }
}
