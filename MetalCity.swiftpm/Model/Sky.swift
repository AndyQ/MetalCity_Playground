...
class Sky: Model {
    ...
    func buildDome() {
        ...
        for phi in stride(from:0, to:90-dphi+1, by:dphi) {
            for theta in stride(from:0, to:360 - dtheta+1, by:dtheta) {
                ...
                v = Vertex(position: SIMD4<Float>(0, 0, 0, 1), normal: SIMD4<Float>(0, 0, 0, 1), color: SIMD4<Float>(0, 0, 0, 1), texCoords: SIMD2<Float>(0, 0))
                v.position = p
                vlist.append(v)
    ...
    func update(){
        ...
        var uniform = PerInstanceUniforms(modelMatrix: translation, normalMatrix: translation.upper_left3x3())
